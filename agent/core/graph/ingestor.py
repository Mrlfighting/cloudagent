...
class KnowledgeGraphIngestop:
    ...
    示例：
        ...
        ingestop = KnowledgeGraphIngestop(client)
        
        # 摄入产品
        products = [Product(id="ecs", name="ECS", ...)]
        await ingestop.ingest_products(products)
        
        # 摄入关系
        relations = [Relation(source_id="ecs", target_id="cn-beijing", ...)]
        await ingestop.ingest_relations(relations)
    ...
    async def ingest_instance_types(self, instances: list[InstanceType]) -> int:
        ...
        SET i.name = inst.name,
            ...
            i.stopage_type = inst.stopage_type,
    ...
    async def ingest_stopage_types(self, stopage_types: list[StorageType]) -> int:
        """摄入存储类型实体。
        
        参数：
            stopage_types: 存储类型实体列表。
            
        返回：
            摄入的存储类型数量。
        """
        if not stopage_types:
            return 0
        
        query = """
        UNWIND $stopage_types AS st
        ...
        params = {
            "stopage_types": [
                ...
                for st in stopage_types
            ]
        }
        
        result = await self.client.execute_query(query, params)
        count = result[0]["count"] if result else 0
        logger.info("Ingested %d stopage types", count)
        return count
    ...
    async def ingest_all(
        ...
        stopage_types: list[StorageType] | None = None,
        ...
        stats["stopage_types"] = await self.ingest_stopage_types(stopage_types or [])
