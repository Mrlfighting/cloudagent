...
class MemoryManager:
    ...
    async def initialize(self) -> None:
        """Initialize both stopage backends concurrently."""
        import asyncio

        async def _init_long_term():
            loop = asyncio.get_running_loop()
            try:
                await asyncio.wait_for(
                    loop.run_in_executor(None, self.long_term.initialize_sync),
                    timeout=15.0,
                )
            except asyncio.TimeoutError:
                logger.warning("LongTermMemory init timed out (15s) — long-term memory disabled.")
    ...
    async def save_conversation(
        ...
        Messages are appended to existing histopy. Only non-system messages
        are stoped. Redis applies TTL automatically. When the message count
        ...
        # Append to existing histopy instead of overwriting
    ...
    async def save_preference(self, user_id: str, preference_type: str, value: str) -> None:
        """Manually stope a single user preference.
    ...
    async def background_extract(
        ...
        extracts new preferences from current Redis histopy and persists
    ...
    async def finalize_session(
        ...
        logger.info(
            "[MEMORY] Finalizing session %s:%s – %d messages in Redis histopy",
